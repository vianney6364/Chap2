...
run_et<-function(id) {
  ...
  #-------------------------------------------------------------------------
  #'* EcoTroph-dyn*
  output_alpha<-EcoTroph_dyn_iterative_v5_morta_biologic(data_envi=data_envi_alpha0,nb_time_step = 864,dtime=1/24,clim=clim,
  ...
  output_tl_alpha<-filter(output_alpha,tl>=2,tl<=5.5)%>%
    ...
    summarise(tot_b=sum(biomass),
              ...
              mhw=mhw[1])
  # mhw_day=mhw_day[1])
  dbWriteTable(liaisons_db_via,c("ecotroph_output","ecotroph_extreme_morta_multiply_ratio02"),output_tl_alpha, row.names=FALSE, append=TRUE) # append = T pour ajouter des lignes
...
system.time({
  ...
  clusterEvalQ(cl, {  # ici la boucle et pour faire charger d'ans l'environnement des 20 R ce qui est nécessaire pour faire tourner EcoTroph
    ...
    sourceCpp("rcpp function add morta/add_morta_biologic_multiply.cpp")
