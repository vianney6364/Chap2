...
run_et<-function(id) {
  ...
  select cell_no,year,year_month,quinzaine,sst_sup_sst_season as sst,npp_interpolated as npp,mhw,percent_mhw_day as mhw_day from observed_forcing.long_term_envi_data_quinzaine
  ...
  output_tl_alpha<-filter(output_alpha,tl>=2,tl<=5.5)%>%
    ...
    summarise(tot_b=sum(biomass),
              ...
              mhw=mhw[1])
  # mhw_day=mhw_day[1])
  dbWriteTable(liaisons_db_via,c("ecotroph_output","ecotroph_extreme_without_mhw"),output_tl_alpha, row.names=FALSE, append=TRUE) # append = T pour ajouter des lignes
