...
run_et<-function(id) {
  ...
  select cell_no,quinzaine,sst as sst,npp as npp from observed_forcing.test_initialisation
  where cell_no =",id,"",sep=""))
  ...
  dummy_year<-rbind(dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year,dummy_year)%>%
    mutate(mhw=F,year=1997,mhw_day=0,time_step=seq(0,287)) %>% 
  ...
  select cell_no,year,quinzaine,sst as sst,npp_interpolated as npp,mhw,percent_mhw_day as mhw_day from observed_forcing.long_term_envi_data_quinzaine
  ...
  output_tl_alpha<-filter(output_alpha,tl>=2,tl<=5.5)%>%
    ...
    summarise(tot_b=sum(biomass),
              ...
              mhw=mhw[1])
  # mhw_day=mhw_day[1])
  dbWriteTable(liaisons_db_via,c("ecotroph_output","ecotroph_ini_test_1998_2006"),output_tl_alpha, row.names=FALSE, append=TRUE) # append = T pour ajouter des lignes
