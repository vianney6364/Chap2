...
run_et<-function(id) {
  ...
  output_tl_alpha<-filter(output_alpha,tl>=2,tl<=5.5)%>%
    ...
    summarise(tot_b=sum(biomass),
              ...
              mhw=mhw[1])
  # mhw_day=mhw_day[1])
  dbWriteTable(liaisons_db_via,c("ecotroph_output","ecotroph_extreme_morta_ratio1"),output_tl_alpha, row.names=FALSE, append=TRUE) # append = T pour ajouter des lignes
